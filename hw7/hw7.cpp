...
std::vector<int> func(std::vector<char> charList, cl::KernelFunctor& sort,
                      std::vector<int> index, ClCmdQueue& queue)
    throw(cl::Error) {
    std::vector<int> ret(index.size());

    cl::Buffer charBuf = queue.makeBuffer(const_cast<char *> (&charList[0]),
                                              charList.size(), queue.ROFlags);
    cl::Buffer indexBuf = queue.makeBuffer(const_cast<int *>(&index[0]),
